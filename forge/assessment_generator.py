...
def _build_markdown(
    ...
    # ── Theory stack section (single vs dual) ───────────────────────────────
    if _has_two_ensembles:
        custom_theories = session.custom_theories or []
        n_a = len(recs)
        ...
        # Ensemble diff
        shared_ids  = sorted(_rec_ids & _cust_ids)
        only_a_ids  = sorted(_rec_ids - _cust_ids)
        only_b_ids  = sorted(_cust_ids - _rec_ids)

        theory_stack_section = f"""## Theory Stack Comparison

### Model A: Recommended Ensemble ({n_a} modules)
{_theory_stack_table(recs)}

### Model B: Custom Ensemble ({n_b} modules)
{_theory_stack_table(custom_theories)}

### Ensemble Difference
| Category | Theories |
|----------|---------|
| Shared | {', '.join(shared_ids) or '—'} |
| Only in Model A | {', '.join(only_a_ids) or '—'} |
| Only in Model B | {', '.join(only_b_ids) or '—'} |

**Structural difference:** The two ensembles differ in their treatment of {', '.join(only_a_ids[:2]) if only_a_ids else 'underlying dynamics'}; Model A emphasises {only_a_ids[0] if only_a_ids else 'the recommended framework'} while Model B substitutes {only_b_ids[0] if only_b_ids else 'the custom framework'}, implying different assumptions about causal pathways in this scenario.

### Module Cascade (Model A)
...
{chart_embeds_theory}"""
        ...
        exec_dual_note = (
            f"\nThis assessment covers two model configurations: the **Recommended Ensemble** "
            f"({n_a} modules) and the **Custom Ensemble** ({n_b} modules). "
            f"The comparison will reveal how modeling assumptions about "
            f"{only_a_ids[0] if only_a_ids else 'divergent theory choices'} "
            f"affect the projected outcome range."
        )
    else:
        theory_stack_section = f"""## Recommended Theory Stack
